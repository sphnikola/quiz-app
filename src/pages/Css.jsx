...
export function Css() {
  ...
  const handleClick = () => {
    ...
    if (!changeText) {
      if (choiceMade === result[1].questions[currentQuestion - 1].answer) {
        setScore(score + 1);
      } else {
        console.log("wrong");
      }
  ...
  return (
    <>
      ...
      rounded-xl md:text-[28px] md:leading-[34px]"
        ...
        <>
          <QuestionPage
            question={result[1].questions[currentQuestion - 1].question}
            currentQuestion={currentQuestion}
            totalQuestion={result[1].questions.length}
          />
          <OptionPage
            listItems={listItems}
